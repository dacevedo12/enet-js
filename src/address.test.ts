...
describe("address host ip", () => {
  it("parses a dotted quad and keeps the port", () => {
    expect.hasAssertions();
    expect(enet.address.setHostIp(anyAddress, LOCALHOST)).toStrictEqual(
      localhost,
    );
  });

  it("returns null for a host that isn't a dotted quad", () => {
    expect.hasAssertions();
    expect(enet.address.setHostIp(anyAddress, "localhost")).toBeNull();
  });
