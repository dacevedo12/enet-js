...
describe("host", () => {
  ...
  describe("create", () => {
    it("creates a server host with address", () => {
      const host = enet.host.create(address, 32, 0, 0);
    ...
    it("creates a client host with null address", () => {
      const host = enet.host.create(null, 1, 0, 0);
      expect(host).not.toBeNull();
      enet.host.destroy(host!);
    });
    ...
    it("returns null when binding to a port already in use", () => {
      const host = enet.host.create(address, 32, 0, 0)!;
      const secondHost = enet.host.create(address, 32, 0, 0)!;
  ...
  describe("connect", () => {
    it("connects to a remote host", () => {
      const server = enet.host.create(address, 32, 0, 0)!;
      const client = enet.host.create(null, 1, 0, 0)!;

      const peer = enet.host.connect(client, address, 1);
    ...
    it("returns null when no peer slots are available", () => {
      ...
      expect(enet.host.connect(host, address, 1)).not.toBeNull();
      expect(enet.host.connect(host, address, 1)).toBeNull();
  ...
  describe("broadcast", () => {
    it("broadcasts a packet to all connected peers", () => {
      const server = enet.host.create(address, 32, 0, 0)!;
      const client = enet.host.create(null, 1, 0, 0)!;
      enet.host.connect(client, address, 1);
