...
const enet_socket_get_address: KoffiFunc<
  (socket: number, address: NativeAddress) => number
> = lib.func(
  "int enet_socket_get_address(ENetSocket socket, _Out_ ENetAddress *address)",
);
...
const enet_socket_get_option: KoffiFunc<
  (socket: number, option: number, value: readonly [number]) => number
> = lib.func(
  "int enet_socket_get_option(ENetSocket socket, int option, _Out_ int *value)",
);
const enet_socket_shutdown: KoffiFunc<(socket: number, how: number) => number> =
  lib.func("int enet_socket_shutdown(ENetSocket socket, int how)");
