...
// Leaves the server's socket holding an acknowledgement for a packet with the given free callback, so the server's next service call runs the callback
const queueAcknowledgement = (
...
// Also leaves a new packet behind the acknowledgement, so the same service call takes a RECEIVE from ENet
const queueAcknowledgementAndPacket = (
  connection: Connection,
  onFree: ENetPacketFreeCallback,
): void => {
  queueAcknowledgement(connection, onFree);
  enet.peer.send(
    connection.peer,
    CHANNEL,
    createPacket(Buffer.from(NEXT_MESSAGE)),
  );
  enet.host.flush(connection.client);
};
